...
vector<struct pipemsg> veclnkid;
...
int main(int argc, char* argv[])
{
	...
	{
		sleep(1);
		printf("veclnkid size:%d\n",veclnkid.size());
		
		for(int i = 0; i < veclnkid.size(); ++i)
		{
			printf("veclnkid confd:%d\n",veclnkid.at(i).fd);
			char buf[1024] = {0};
			...
			int len = send(veclnkid.at(i).fd, buf, strlen(buf), 0);
		}
